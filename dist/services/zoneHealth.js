...
exports.analyzeZoneHealth = analyzeZoneHealth;
const dnsResolver_1 = require("./dnsResolver");
// Validate SOA record
function validateSOA(records) {
    const issues = [];
...
// Validate NS records
function validateNS(records, domain) {
    const issues = [];
    if (!records || records.length === 0) {
        issues.push({
            severity: 'critical',
...
// Validate MX records
function validateMX(records, domain) {
    const issues = [];
    if (!records || records.length === 0) {
        issues.push({
            severity: 'info',
...
function validateTXT(records, domain) {
    const issues = [];
    let hasSPF = false;
    let hasDKIM = false;
    let hasDMARC = false;
    ...
    // Check for SPF
    const spfRecords = records.filter(r => r.data.toLowerCase().includes('v=spf1'));
    ...
    const dmarcRecords = records.filter(r => r.data.toLowerCase().includes('v=dmarc1'));
...
function calculateScore(issues, summary) {
    let score = 100;
    // Deduct for issues
    ...
    if (summary.nsCount >= 2)
        score += 5;
    if (summary.hasSPF)
        score += 5;
    if (summary.hasDMARC)
        score += 5;
    if (summary.mxCount >= 2)
        score += 3;
...
async function analyzeZoneHealth(domain) {
    const { records, errors } = await (0, dnsResolver_1.getAllDNSRecords)(domain);
    const issues = [];
...
//# sourceMappingURL=zoneHealth.js.map